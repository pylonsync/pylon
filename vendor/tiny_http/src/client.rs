...
impl ClientConnection {
    ...
    pub fn new(
        ...
        // Pylon patch: `getpeername` on a socket whose peer vanished
        // mid-accept can return a short sockaddr, which Rust's std
        // converts with an `assert!` — a hard panic that kills this
        // connection thread (observed on macOS with ::1 connections,
        // taking WebSocket handshakes and keep-alive requests with
        // it). Treat a panicking peer_addr as "address unknown".
        let remote_addr = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            read_socket.peer_addr()
        }))
        .unwrap_or(Ok(None));
