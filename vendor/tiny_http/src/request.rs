...
impl Request {
    ...
    /// Pylon patch: like [`Request::upgrade`], but returns the
    /// connection's reader and writer halves SEPARATELY instead of
    /// fusing them into one `Box<dyn ReadWrite>`.
    ///
    /// Rationale: a fused stream forces every consumer into strictly
    /// alternating read/write (one `&mut` owner). A WebSocket server
    /// that broadcasts needs concurrent halves — a reader thread that
    /// may block in `read()` indefinitely, and an independent writer
    /// thread that flushes outbound frames the moment they're queued.
    /// The halves already exist internally (`upgrade()` builds its
    /// `CustomStream` from them); this just stops hiding them.
    ///
    /// The `notify_when_responded` handshake (tiny-http's signal to
    /// resume connection bookkeeping) is attached to the READER half:
    /// an upgraded connection is over when its read loop exits, and
    /// callers drop the writer alongside it.
    pub fn upgrade_split<R: Read>(
        ...
        let reader: Box<dyn Read + Send> = if let Some(sender) = self.notify_when_responded.take()
        {
