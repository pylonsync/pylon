...
Measured on a 2024 laptop (Apple M-series, 16 GB RAM), in-memory SQLite. Numbers are per-process throughput — SQLite is single-writer, so vertical scaling of writes is bounded. Reads scale across a connection pool.
