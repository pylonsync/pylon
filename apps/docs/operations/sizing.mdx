...
Cloud supports multiple application machines for Postgres projects. It uses
the managed PylonSync relay for cross-machine realtime events. SQLite projects
must stay on one application machine. Resize the machine or switch to Postgres
before you add replicas.

For multiplayer apps with sticky shard connections, latency to your players matters. Set the workspace region accordingly when you sign up.
