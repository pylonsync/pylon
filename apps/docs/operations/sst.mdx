...
<Warning>
  **S3 file storage is not currently wired into the runtime.** File-storage
  selection (`select_from_env` in `crates/storage/src/files.rs`) only recognizes
  `PYLON_FILES_PROVIDER=stack0` and `local` — an `s3` value **silently falls back
  to local disk**. On a stateless Fargate container that means uploads land on
  ephemeral container disk and are lost on every redeploy or scale-in. There is
  no `file_storage` manifest plugin; file storage is configured entirely through
  `PYLON_FILES_PROVIDER` + the provider's env vars.

  Until S3 is wired, either use `PYLON_FILES_PROVIDER=stack0` (see
  [Deploy → Required environment](/operations/deploy#required-environment)) for
  externalized files, or use the single-replica **EFS + SQLite** shape below,
  which keeps uploads on the mounted EFS volume. The `PYLON_S3_*` env vars and the
  `S3Config` struct exist in the codebase but aren't reachable from the runtime's
  storage selector yet.
</Warning>
