...
Jobs and workflows need no Redis. They use the shared Aurora database and can fail over between replicas. Their handlers must be idempotent because execution is at least once.
...
Shards spread across the service's machines through the shard directory in Postgres. Set `PYLON_REPLICA_ID` per task, `PYLON_SHARD_ADVERTISE_URL` to each task's private address, and the same `PYLON_SHARD_TICKET_SECRET` on all of them; see [Shards across machines](/concepts/shards#shards-across-machines).
...
## Compared to Stack0 Cloud

If you are not committed to AWS, [Stack0 Cloud](/cloud) gives you the same managed Postgres, S3, TLS, and WebSocket-aware load balancing with one CLI command (`pylon deploy --target cloud`) and per-use pricing. SST is the right choice when:
...
A bare-bones non-production stack runs about $30/mo. Production with NAT and multi-AZ runs about $80 to $150/mo, depending on traffic. For low-traffic apps, [Stack0 Cloud](/cloud) is often cheaper than the AWS minimums.
