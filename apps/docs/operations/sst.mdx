...
Shards spread across the service's machines through the shard directory in Postgres. Set `PYLON_SHARD_ADVERTISE_URL` to each task's private address so tasks can pass connections to each other; see [Shards across machines](/concepts/shards#shards-across-machines).
