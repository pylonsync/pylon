...
**Fix**: shard rooms. On Workers each Pylon room maps to one (sticky) Durable Object, so split a hot "global chat" room into many smaller rooms (e.g. per-region) to spread load across DOs instead of pinning everyone to one.
