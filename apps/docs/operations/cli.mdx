...
The `pylon` binary covers every operator task: the dev loop, deploys, secrets, logs, db, data, domains, deployments, members, and project health. Anything you do from the Stack0 Cloud dashboard, you can do from the CLI and pipe to other shell tools.
...
`PYLON_CLOUD_URL` overrides the default `https://www.pylonsync.com` for staging or self-hosted Stack0 Cloud installs.
...
`pylon projects create <slug>` provisions a project on Stack0 Cloud without the
...
pins the slug as your local context, and prints the live `https://<slug>.stack0.app` URL.
...
(`{"ok":true,"slug":"my-app","status":"running","url":"https://my-app.stack0.app",...}`),
...
```bash
pylon deploy                       # default → Stack0 Cloud (packages local
...
```bash
pylon jobs                         # 50 most recent jobs: name, status, tries, start, first error line
pylon jobs failed                  # only failed jobs
pylon jobs dead                    # the dead-letter queue
pylon jobs stats                   # pending / running / completed / failed / dead counts
pylon jobs get <id>                # one job with the full error text
pylon jobs --status retrying --queue mail --limit 200
```

Reads `/admin/jobs` on the project's machine through Stack0 Cloud. Owner / admin only. A cron that stopped writing shows up here as a failed row with its function name and error.
