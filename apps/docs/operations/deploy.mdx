---
title: Deploy
description: "Deploy Pylon to a single VPS, AWS ECS, Cloudflare Workers, or Stack0 Cloud."
---

Pylon runs as a Rust binary (plus Bun, which runs your TypeScript and SSR) behind a TLS-terminating reverse proxy. This page covers the supported self-hosted deploy shapes and the experimental Workers path. To skip self-hosting, [Stack0 Cloud](/cloud) hosts the same binary.

Each shape can run your source (`pylon start app.ts`) or a [production build](/operations/build) (`pylon start dist`). The production build needs no source tree and no `node_modules` on the server.
...
PYLON_SHARD_WS_MAX_PER_IP=0                   # shard WS connections per client IP (default 64; 0 = no cap)
PYLON_SHARD_TICKET_SECRET=...                 # shard ticket key; same value on every machine (default: derived from PYLON_JWT_SECRET)
...
Reverse proxy (Caddy or nginx) forwards `:443` → `:4321` plus WebSocket upgrades for `/ws` → `:4322`, `/events` (SSE) → `:4323`, and shard WS → `:4324`. Shard WebSockets are also served at `/shard` on the main port, and the TypeScript and Swift shard clients use that path by default, so a proxy that forwards WebSocket upgrades on `:443` needs no shard rule. `/shard` needs plain HTTP on `:4321` (TLS terminated at the proxy).

The shard WebSocket allows 64 connections per client IP by default (`PYLON_SHARD_WS_MAX_PER_IP`). On `/shard` the client IP comes from the trusted proxy headers; on `:4324` behind a proxy, every connection comes from the proxy's address, so set `PYLON_SHARD_WS_MAX_PER_IP=0` or a value above your player count. `pylon deploy --target systemd` emits a matching nginx or Caddy reverse-proxy config alongside the unit file (generated by `crates/runtime/src/tls.rs`).
...
Pylon supports multiple processes behind a load balancer. Use Postgres for shared entities, auth state, jobs, workflows, and sync state. Configure `PYLON_CLUSTER_BUS` for live change, presence, and CRDT fanout between self-hosted processes. Stack0 Cloud configures its managed relay for you.
...
See [Horizontal scaling](/operations/scaling) for the full setup and failure model.

For multi-region on Stack0 Cloud, set the workspace region during signup. Self-hosted multi-region is a manual setup: one Pylon process per region, with app-level routing.
...
To ship only the build output, see the multi-stage Dockerfile in [Production build](/operations/build#docker).
