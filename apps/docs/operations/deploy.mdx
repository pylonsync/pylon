...
Reverse proxy (Caddy or nginx) forwards `:443` → `:4321` plus WebSocket upgrades for `/ws` → `:4322`, `/events` (SSE) → `:4323`, and shard WS → `:4324`. Behind a proxy, every shard connection comes from the proxy's address, so set `PYLON_SHARD_WS_MAX_PER_IP=0` (or a value above your player count); the default cap is 64 per IP. `pylon deploy --target systemd` emits a matching nginx or Caddy reverse-proxy config alongside the unit file (generated by `crates/runtime/src/tls.rs`).
