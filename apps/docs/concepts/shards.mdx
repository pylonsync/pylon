...
On the client, read the table the frames build:

```ts
import { connectShard } from "@pylonsync/react";

const client = connectShard(shardId, { subscriberId, ticket });
client.onReplication((entities, summary) => {
  for (const id of summary.spawned) addMesh(id);
  for (const id of summary.despawned) removeMesh(id);
});

function frame() {
  for (const e of client.entities.entities.values()) {
...
`useShard` returns the same table as `entities`. The table and its decoder
also ship without React in `@pylonsync/realtime` (`EntityTable`). In Swift,
`ShardClient<NoSnapshot, Input>` has `entities` and a `replication()`
stream.
