...
**Durability.** Cron jobs are backed by a persistent job store. A fire that is in flight survives a restart, and the schedule resumes after a redeploy.

<Warning>
  **Multiple replicas.** Each running Pylon process schedules crons independently. On a shared datastore (Postgres, the Pylon Cloud default), the runtime takes a per-minute lease, so each cron fires exactly once per tick across all replicas. This costs no extra work on your part. On per-replica SQLite, there is no shared lease, so each replica fires every tick. Keep those handlers idempotent (guard on a row you write, since most maintenance is naturally idempotent already), or run the scheduled work on a single machine. A single-machine app always fires exactly once.
</Warning>
