...
return value), exactly like a JSON-only call — `db.streamFn` handles both. This is what
makes serving MCP from an action work: MCP clients advertise SSE support on every POST
