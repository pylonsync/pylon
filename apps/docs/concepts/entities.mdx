...
Edit `app.ts`, save — `pylon dev` picks up the change and runs a live migration. Pylon's storage layer plans the diff (add column, drop index, etc.) and applies it to the SQLite file. Destructive operations (dropping a column that has data) require you to bump `manifest.version`.
