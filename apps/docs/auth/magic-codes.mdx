...
On Stack0 Cloud, magic-link email is included. You need no provider config for the built-in sender. Configure your own domain in `Settings → Email` for production to keep deliverability high.
...
To customize, write your own action that calls `magic/send` server-side and sends your own templated email. The plugin system also supports replacing the email transport. See [Plugins → Integrations](/plugins/integrations).
