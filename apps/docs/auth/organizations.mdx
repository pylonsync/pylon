...
The equivalent helper form is `auth: auth({ orgRoles: [...] })`. Role slugs must match `[a-z][a-z0-9_-]{0,63}`. Built-ins are always available and must not be redeclared.

Custom roles are exact-match labels: they do **not** inherit `member`, `admin`, or any other permission. The framework still reserves member management for owners/admins. Grant application permissions explicitly with `auth.hasRole("reviewer")` in policies or `ctx.requireMember(orgId, { role: ["reviewer"] })` in functions.
...
- **Only Owners can promote to Owner.** Admins can't self-promote. `400 BAD_ROLE` lists the built-in and manifest-declared roles when a role is unknown; `403 FORBIDDEN` is returned when a non-owner tries to promote a member to owner.
...
A session can have an active org. The active tenant is what policies, change-event filters, and `ctx.auth.tenantId` see. Its exact membership role is exposed consistently by `/api/auth/me`, SSR `PageAuth.roles`, policy `auth.hasRole(...)`, and function `ctx.auth.roles`:
