---
title: Organizations
description: "Multi-tenant orgs with owner/admin/member roles, email invites, and active-tenant session state."
---
...
```rust
struct Org {
  id: String,         // org_<24-char-base64url>
  name: String,
  created_by: String, // immutable — distinct from "owner"
...
struct Invite {
  id: String,         // inv_<24-char-base64url>
  org_id: String,
  email: String,      // lowercased on store
  role: OrgRole,
  invited_by: String,
  token_hash: String, // Argon2 hash; plaintext only ever in the email
  token_prefix: String, // first 8 chars for display in management UIs
  created_at: u64,
