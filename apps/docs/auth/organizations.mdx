...
<Note>
  This page documents the framework's org primitives (Org / OrgMember / OrgInvite entities + `/api/auth/orgs/*` routes). For a permission system on top (declarative `permissions: { "x.y": ["owner", "admin"] }` + `requirePermission(ctx, "x.y")`) and optional team support, see [`@pylonsync/organizations`](/plugins/organizations).
</Note>
