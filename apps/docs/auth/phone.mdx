...
The `dev_code` field is returned whenever `PYLON_DEV_MODE=true` or the SMS transport returned an error. A misconfigured Twilio account then does not lock anyone out during early development. In production with a working Twilio config, `dev_code` is omitted and only the SMS carries the code.
