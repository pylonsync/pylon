...
The implementation covers user provisioning and the `userName eq "..."` filter that identity providers use for lookup. It does not yet cover Groups, Bulk, or the full SCIM filter grammar. This surface supports the standard Okta and Azure AD provisioning flow for creating and deactivating users.
