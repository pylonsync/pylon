---
title: Sessions
description: "How Pylon manages opaque tokens, refresh, revocation, persistent storage, and cookies."
---

A Pylon session is an opaque 256-bit token (prefixed `pylon_`) that maps to a `Session { token, user_id, expires_at, device, created_at, tenant_id }`. Bearer headers and `HttpOnly` cookies both resolve through the same `SessionStore`.

Because session tokens are opaque and server-resolved, they need no signing key and can be revoked immediately.
...
Sessions live in memory by default. That is fine for development, but users get logged out on every restart. For production, point Pylon at a SQLite file:

```bash
PYLON_SESSION_DB=/var/lib/pylon/sessions.db
```

Now sessions survive restarts, deploys, and crashes. The SQLite file is a write-through cache: reads still hit memory, and every save or remove writes to disk.

On **Stack0 Cloud**, persistent sessions are configured automatically.
...
The full token is never returned. The response shows only the first 8 chars for display. Power users can audit active sign-ins and revoke individual ones.
...
When both a cookie and an `Authorization: Bearer` header are present on the same request, the bearer header wins.
...
The server verifies membership before committing. It looks up an `OrgMember { userId, orgId }` row and returns 403 `NOT_A_MEMBER` if it does not exist. Clients cannot impersonate an org they do not belong to.
...
Guests have a stable `user_id` (so their cart persists across page loads), but `is_authenticated()` returns false. `AuthMode::User` rejects them, so guests cannot access user-only routes.
...
This is the admin-only path. Never expose it without admin auth. In non-dev mode, non-admin requests get 403 `FORBIDDEN`. Use it for:
...
Cloud runs this hourly. Self-hosted, the on-demand check is usually enough. Leaving expired rows in the table briefly does no harm.
...
Custom backends (Redis, DynamoDB, etc.) are a few lines of Rust. Implement the trait and pass it via `SessionStore::with_backend`. See `crates/runtime/src/session_backend.rs` for the SQLite reference impl.
