...
| WebAuthn / passkeys | ES256 + Ed25519 verification primitives in `pylon_auth::webauthn`. **HTTP endpoints not yet wired** — apps compose their own register/login routes from `verify_assertion` + `PasskeyStore`. |
...
| SIWE (Sign-In With Ethereum) | EIP-4361 message validation + nonce stash. **Signature verification is opt-in** via `SiweSignatureVerifier` trait — apps wire k256 themselves to keep crypto opt-in. |
| OIDC Provider | Discovery doc / JWKS / AuthCode types in `pylon_auth::oidc_provider`. **HTTP endpoints not yet wired**. |
| SCIM 2.0 | Schema types + bearer auth helper in `pylon_auth::scim`. **HTTP endpoints not yet wired**. |
