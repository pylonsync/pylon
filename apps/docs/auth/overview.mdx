...
| SIWE (Sign-In With Ethereum) | EIP-4361 message verification with per-address nonce stash; wallet-native passwordless |
| OIDC Provider | Run pylon as the IdP — discovery doc + JWKS + auth-code flow at `/.well-known/openid-configuration` and `/oidc/...` |
| SCIM 2.0 | Auto-provision users from Okta / Azure AD / Workday at `/scim/v2/Users` (bearer auth via `PYLON_SCIM_TOKEN`) |
