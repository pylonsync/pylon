...
| OIDC Provider | Discovery doc + JWKS endpoints (`/.well-known/openid-configuration`, `/oidc/jwks`) — `PYLON_OIDC_JWKS_RSA_N/_E` to publish RSA public key. |
| SCIM 2.0 | `/scim/v2/Users` (POST/GET/DELETE) — bearer auth via `PYLON_SCIM_TOKEN`. SCIM `active=false` → soft delete. |
