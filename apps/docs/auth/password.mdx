...
Some apps need a password flow: shared devices, no email access, or regulatory requirements. Pylon ships built-in email + password auth. Passwords are hashed with Argon2id (the OWASP-recommended algorithm). Failed logins run a dummy hash so timing does not leak whether the email exists.
...
The user is created and signed in, with no separate "verify your email" step. If you need email verification, call `/api/auth/email/send-verification` after sign-in. The user can keep using the app while their email is unverified. Gate sensitive flows on `User.emailVerified`.
...
The error message intentionally does not say which one is wrong. The server runs the password hash check even when the email does not exist, so timing analysis cannot enumerate accounts.
...
Pylon uses [`argon2`](https://crates.io/crates/argon2) with the Argon2id variant, the winner of the Password Hashing Competition and the OWASP recommendation. It resists both side-channel attacks (Argon2i) and GPU-based brute-force (Argon2d) by design.
...
Pylon ships a built-in "forgot password" flow. Two endpoints email a
single-use reset link and change the password after the user clicks it:
...
across the registered and not-registered paths, so it cannot be used to
...
If you do not want a password-reset UI, run users through the
[magic-code](/auth/magic-codes) flow instead. It mints a session without a
password. Then update `User.passwordHash` yourself from an authenticated
action.
...
- **`/login` is rate-limited by default** — the built-in per-IP/per-email limiter throttles the credential bucket (login, register, reset, TOTP verify) and returns `429 RATE_LIMITED` with a `retry_after` hint on abuse. Stack0 Cloud layers additional per-IP limiting at the edge.
