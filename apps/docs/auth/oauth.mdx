...
On **Stack0 Cloud**, set these in `Settings → Environment` per workspace.
...
Pylon captures the provider's profile picture (OIDC `picture` claim, GitHub
...
On later sign-ins, Pylon looks up the user by `(provider, provider_account_id)`, not by email, so a user who renamed their email keeps their account.
