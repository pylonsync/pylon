...
<Tip>
  **Just want a working app?** Skip the manual setup with one command:
  ...
  That scaffolds a single full-stack Pylon app — SSR React 19 frontend + API served from one port by `pylon dev`, no Next.js, no monorepo. Templates: `default` (SaaS), `todo`, `chat`, `shop`, `consumer`, `waitlist`, `ai-chat`, and more — run `npm create @pylonsync/pylon@latest` with no args to pick one. Every app ships with a test harness (`pylon test`) wired up.

  Ready to ship? `pylon deploy` (see [Deploy](/operations/deploy)).
</Tip>
