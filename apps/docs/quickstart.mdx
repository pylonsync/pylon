...
<Tip>
  **Just want a working app?** Skip the manual setup with one command:
  ...
  That scaffolds a Pylon backend + Next.js 16 + React 19 frontend + shared UI package wired together. Templates: `barebones`, `todo`, `consumer`, `chat`. Platforms: `web`, `ios`, `mac`, `expo`.
