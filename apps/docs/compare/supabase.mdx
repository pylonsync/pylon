...
The biggest delta: Pylon doesn't expose raw SQL. If your app relies on custom Postgres queries (CTEs, complex joins, window functions), you'll either:

1. Keep using Postgres and ship those queries as Pylon `actions` that hit the DB directly via `ctx.dbConn.query(...)` (`postgres-live` feature), or
2. Express the logic in TypeScript inside an action

Most CRUD apps don't need anything beyond Pylon's query interface.
