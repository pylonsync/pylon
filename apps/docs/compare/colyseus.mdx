...
[Colyseus](https://colyseus.io) is a widely used open-source multiplayer game server framework for Node.js. It nails the "rooms + tick-based state sync" model. Pylon's game shards (`Shard<S: SimState>`) are inspired by Colyseus's `Room` API.
