...
- **Convex Vector Search at scale** — Convex's vector indexes are approximate and built for large tables. Pylon's built-in [vector search](/concepts/vector-search) (`field.vector(dims)` + `ctx.llm.embed` + `ctx.db.vectorSearch`) is exact k-NN — perfect recall, zero index maintenance, right to ~100k rows per entity; past that Convex's ANN wins.
...
- **Batteries included** — TOTP, audit log, webhooks, rate limiting, file storage, faceted search, and vector search ship in the binary, configured from the manifest or env — no marketplace of third-party components.
