...
- **Cron + scheduled functions** — Convex has clean durable primitives for scheduled jobs. Pylon has cron, queues, and workflow primitives too, but Convex's scheduler UI is more polished.
...
| Convex scheduled functions | `cron(...)`, scheduled workflows, or background jobs |
