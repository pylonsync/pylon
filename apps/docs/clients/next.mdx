...
- **`createPylonServer`** — server-side fetch helpers (`pylon.requireAuth()`, `pylon.json()`) that forward the user's session cookie
...
Works with App Router (Next.js 13.4+); Pages Router users can still use [`@pylonsync/react`](/clients/react) directly.
...
export default async function DashboardPage() {
  // 401 → redirect to /login automatically.
...
| --- | --- | --- |
| `requireAuth()` | `{ user, session }` (or redirects) | Gate Server Components / Server Actions |
| `getAuth()` | `{ user, session } \| null` | Read auth without redirecting |
...
For Client Components, the package exports a same-origin `api()` helper that the `useQuery` / `useMutation` hooks build on top of. Most apps don't construct one directly — just `import { api } from "@pylonsync/next"`:

```tsx
"use client";
import { api } from "@pylonsync/next";
