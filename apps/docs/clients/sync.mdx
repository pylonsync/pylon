...
For a per-call ergonomic surface, prefer the React [`db.useMutation`](/clients/react) hook — it exposes `loading` / `data` / `error` for the specific call without reaching into the queue.
...
If you mutate a session via a path that bypasses the framework's auth surface (rare — e.g. directly writing to SessionStore from a Rust plugin), call `engine.notifySessionChanged()` to force the refresh.
