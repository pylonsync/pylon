...
When the token changes (sign-in, sign-out, switch user) and you mutated it via your own client rather than the SDK helpers below, call `engine.notifySessionChanged()` so the engine refreshes `/api/auth/me` and resets the replica if the visible set changed. The helpers do this for you.
