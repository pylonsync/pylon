...
The RN replica persistence mirrors the crash-safety model of the web client's IndexedDB path. The engine writes the replica before the cursor advances. A crash mid-pull cannot leave the cursor ahead of durable state.
