---
title: React SDK
description: "Use @pylonsync/react hooks for queries, mutations, sessions, search, presence, and multiplayer."
---
...
### `db.useMutation` — server function with optimistic updates
...
When the server moves the player to another shard
(`ctx.shards.transfer`), the hook reconnects there on its own; its
`shardId` field is the shard it is connected to. When a deploy moves the
shard itself to another machine, the hook reconnects the same way and its
`shardId` stays. See
[Move a player to another shard](/concepts/shards#move-a-player-to-another-shard)
and [Deploys](/concepts/shards#deploys).
...
These hit the local store optimistically (where applicable) and the server in the background. They use the same path as the hooks.
