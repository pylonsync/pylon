...
The Swift SDK gives you full TypeScript-equivalent functionality on Apple platforms (and Linux for server-side Swift). Auth, entity CRUD, optimistic mutations, real-time sync with offline replay, multiplayer shards, Loro CRDTs, and SwiftUI hooks are all native, with no JavaScript bridge.
...
When the server moves the player to another shard (`ctx.shards.transfer`),
the client reconnects there with the ticket the server sent, and
`shard.shardId` changes. `transfers()` reports each move. When a deploy
moves the shard itself to another machine, the client reconnects the same
way, `shardId` stays, and `transfers()` reports nothing. Set
`ticketProvider` to get a ticket per connection attempt, for the shard it
connects to:
...
let cfg = ShardClientConfig(
    baseURL: URL(string: "https://your-app.com")!,
    subscriberId: userId,
    ticketProvider: { shardId in try await api.zoneTicket(shardId) }
)
for await zone in await shard.transfers() {
    print("now in \(zone)")
}
```
...
- **Codable everywhere:** entities decode into your Swift structs at the boundary; no JSON-as-`Any` values
- **Background sessions:** Apple's `URLSession.background` works
