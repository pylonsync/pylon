...
const init = await fetch("/api/files/init", {
  ...
  body: JSON.stringify({
    ...
    mimeType: file.type,
    size: file.size,
    visibility: "private", // or "public"; see below
  }),
}).then((r) => r.json());
...
```ts
import { uploadFile } from "@pylonsync/react";
...
`visibility` on `/api/files/init` decides who can read the file through `GET /api/files/<id>` on backends that track ownership (local disk, a private S3 bucket):

| Value | Who can read |
| --- | --- |
| `"private"` (default) | The uploader, an unscoped admin, or anyone holding a [signed URL](/concepts/functions#signed-file-urls) |
| `"public"` | Anyone, signed in or not |

Use `"public"` for files every visitor sees, such as feed photos or listing images. Writes and deletes stay with the uploader either way. Any other value is refused with `400 INVALID_VISIBILITY`. On Stack0 and on a public bucket, `stored.url` is a CDN URL that anyone can load, whatever the visibility.
