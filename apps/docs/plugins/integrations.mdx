---
title: Integrations
description: "File storage, transactional email, Stripe billing, outbound webhooks, feature flags — how each is wired in Pylon today."
---
...
The server validates these at boot and fails fast if `PYLON_FILES_PROVIDER=stack0` but the Stack0 key is missing. (`local` and `stack0` are the providers that ship today.)
