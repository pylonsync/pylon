...
| Variable | Purpose |
| --- | --- |
| `STRIPE_SECRET_KEY` | API requests (set per Stack0 Cloud project's Secrets UI). |
| `STRIPE_WEBHOOK_SECRET` | Signature verification on `/api/webhooks/stripeWebhook`. |
| `PYLON_PUBLIC_URL` | Auto-set by Stack0 Cloud. Drives URL allowlist for `success_url`/`cancel_url`. |
