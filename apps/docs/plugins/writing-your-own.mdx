...
The runtime has no manifest lookup or dynamically loaded plugin bundle. **To ship your own today, fork the runtime** (`crates/runtime`), add your `reg.register(...)` call, and build a custom `pylon` binary. Dynamic plugin loading and a wired `definePlugin` remain on the roadmap.
