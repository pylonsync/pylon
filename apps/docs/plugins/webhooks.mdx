...
description: "Deliver outbound webhooks with Svix-compatible signatures, retries, and overlapping secret rotation."
---
...
Tenant-scoped policy: `auth.tenantId == data.applicationId or auth.is_admin`. Apps that need a stricter policy (for example, only the owner role) override this in their manifest.
...
This is a read-only entity. Customers can view their endpoint's delivery history, but they cannot change it.
