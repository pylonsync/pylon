...
**npm plugins** (imported, not declared in `manifest.plugins`): `@pylonsync/stripe` (web billing: checkout, portal, webhook, plan limits, trials), `@pylonsync/revenuecat` (native in-app purchases: the RevenueCat webhook and a server-verified `syncEntitlements` action write one `RcEntitlement` row per user + entitlement that syncs to every device; gate with `hasEntitlement(rows, "pro")`). Native sign-in for mobile apps: `POST /api/auth/native/apple|google` verifies a platform id_token against the provider's JWKS (audiences from `PYLON_APPLE_NATIVE_CLIENT_IDS` / `PYLON_GOOGLE_NATIVE_CLIENT_IDS`); `@pylonsync/react-native` exposes `nativeSignIn`, `guestSession`, `sendEmailCode`/`verifyEmailCode`, `signOut`, `deleteAccount`.
