import { SyncEngine, createSyncEngine, type Row, type SyncEngineConfig } from "@agentdb/sync";
...
/**
 * Initialize the agentdb client. Call once at app startup.
 *
 * ```ts
 * import { init } from "@agentdb/react";
 ...
 * ```
 *
 * Or with the spread shorthand:
 * ```tsx
 * const { subscribe, getSnapshot } = db.useQuery("Todo");
...
export const db = {
  ...
  setPresence(data: Record<string, unknown>) {
    (getSync() as unknown as { setPresence: (d: Record<string, unknown>) => void }).setPresence(data);
  },

  /** Publish to a topic. */
  publishTopic(topic: string, data: unknown) {
    (getSync() as unknown as { publishTopic: (t: string, d: unknown) => void }).publishTopic(topic, data);
  },
};
