...
// different hosts:
//
//   apps/control-plane   → cloud.stack0.dev   (the product; owns /signup)
...
// the product; from cloud.stack0.dev it resolves to the same origin the visitor
...
export const ACCOUNT_ORIGIN = "https://cloud.stack0.dev";

/**
 * Absolute URL on the account host. Pass a root-relative path.
 *
 * `accountUrl("/signup?plan=team")` → `https://cloud.stack0.dev/signup?plan=team`
 */
