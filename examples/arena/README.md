...
- **Live query fan-out.** A single `db.useQuery("Dot")` subscription
  ...
  through the change log without Redis or a separate realtime layer.
- **End-to-end latency HUD.** The client times every outgoing mutation
  until it observes its own row update. The top-left HUD shows live p50
  and p95 latency.
- **Throughput.** Mutations/sec counter shows the actual write rate
  driven by this browser + bots.
- **Client-side interpolation.** The server stores positions, and the
  client interpolates between updates. A 20 Hz update rate handles
  hundreds of concurrent dots.
...
The latency HUD turns red when p95 exceeds 100ms, where the canvas starts to feel laggy. Open multiple tabs and increase the bot count to watch p50 and p95 as concurrency grows. Live-query fan-out uses the same code path at every subscriber count.
