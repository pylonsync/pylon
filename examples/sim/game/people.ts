...
// One capsule figure (~1.0 unit ≈ 1.7 m tall), base at y=0.
const FIGURE_GEO = new THREE.CapsuleGeometry(0.15, 0.7, 3, 6).translate(0, 0.5, 0);
...
export class People implements GameSystem {
  ...
  setRoads(
    ...
    const mesh = new THREE.InstancedMesh(FIGURE_GEO, FIGURE_MAT, this.walkers.length);
  ...
  update(ctx: FrameCtx): void {
    ...
    if (!mesh || this.walkers.length === 0) return;
    for (let i = 0; i < this.walkers.length; i++) {
