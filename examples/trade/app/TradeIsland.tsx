...
// Client island = the SSR-safe Pylon bootstrap. The Trade UI drives the sync
// engine (live queries, WebSocket fan-out, a client-side market maker), none
...
// TradeApp: its first action (seedMarket) needs a signed-in caller, so the app
...
async function bootstrap(): Promise<void> {
  ...
  try {
    ...
    // TradeApp's ensureGuest reads storageKey("token") + storageKey("user").
    ...
    // Pylon may not be reachable yet — TradeApp's hooks retry.
  }
}
