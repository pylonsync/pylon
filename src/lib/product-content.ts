...
export default function ProductPage({ params }) {
  return <ProductView slug={params.slug} />;
	...
	cloud: {
		slug: "cloud",
		navLabel: "Pylon Cloud",
		category: "Pylon Cloud",
		title: "Push to GitHub. It's live.",
		tagline:
			"Pylon Cloud runs the same binary you run locally. Connect a repo and every push deploys. Resize machines, add replicas, expand regions, bring your domain, and turn on SSO from one dashboard.",
		highlights: [
  ...
  → https://acme.pyln.dev`,
		...
		sections: [
			{
				...
				body: "There's no special cloud build of your app. Pylon Cloud runs the identical runtime you run on localhost, so what passes in dev behaves in production. Connect a repo for push-to-deploy with preview environments per pull request, or cut releases manually with pylon deploy.",
			},
			{
		...
		metaTitle: "Pylon Cloud: push-to-deploy managed hosting for Pylon apps",
		metaDescription:
			"Connect a repo and push. Pylon Cloud runs the same binary you run locally with scaling, global regions, custom domains, SSO, snapshots, and one dashboard.",
