	...
	return (
		<div className="relative mx-auto mt-8 w-full max-w-[1000px] select-none overflow-hidden sm:mt-14 lg:overflow-visible">
			...
			<svg
				...
				className="pylon-runtime-art w-[160%] max-w-none -translate-x-[18.75%] lg:w-full lg:translate-x-0"
			>
				<defs>
					...
					return (
						<g key={service.label}>
							<line
								...
								x2={leaderEnd}
								y2={y}
								stroke={isActive ? "var(--color-brand)" : "var(--color-rule)"}
								strokeDasharray={isActive ? "1 0" : "2 5"}
								style={{
									transition: "stroke 420ms ease, stroke-dasharray 420ms ease",
								}}
							/>
							...
							>
								<path
									d={diamondPath(x, y, 20, 11)}
									fill={isActive ? "var(--color-brand)" : "var(--color-paper)"}
									stroke={
								...
								<circle
									...
									r="2.2"
									fill={
										isActive ? "var(--color-brand-fg)" : "var(--color-ink-4)"
									}
								/>
							</g>
						</g>
			...
			{/* Desktop labels sit on the traces. Small screens get one compact
			    caption so the art remains readable instead of shrinking five chips. */}
			<div className="-mt-1 min-h-12 text-center lg:hidden" aria-live="polite">
				<p className="font-mono text-[10.5px] uppercase tracking-[0.14em] text-[var(--color-brand)]">
					{activeService.label}
				</p>
			...
			>
				<span
					...
					style={{
						...
						background: active ? "var(--color-brand)" : "var(--color-paper)",
						color: active ? "var(--color-brand-fg)" : "var(--color-ink-4)",
						boxShadow: active
