use pylon_kernel::{AppManifest, Diagnostic, Severity, Span};
use pylon_schema::{Entity, Field, FieldType, Index, Schema};
...
pub fn validate_all(manifest: &AppManifest) -> Vec<Diagnostic> {
    ...
    let mut diagnostics = pylon_schema::validate_manifest_version(manifest);
...
mod tests {
    ...
    fn field_type_rename() {
        ...
        let f: pylon_kernel::ManifestField = serde_json::from_str(json).unwrap();
