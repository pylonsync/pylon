...
fn print_command_help(cmd: &str) -> bool {
    match cmd {
        ...
        "start" => {
            ...
            println!("  pylon start <dir> [--port <n>]");
            println!();
            println!("The same server as `dev`, without the file watcher. With app.ts it runs the");
            println!("source. With <dir> it runs a `pylon build` artifact, and data paths");
            println!("(PYLON_DB_PATH, default pylon.db) resolve outside that directory.");
            println!();
            println!("Options:");
            println!("  -p, --port <n>    Port to listen on (default: 4321)");
            println!("  -h, --help        Show this help");
        ...
        "build" => {
            ...
            println!();
            println!("Usage:");
            println!("  pylon build [app.ts] [--out <dir>] [--compile] [--json]");
            println!();
            println!("Writes pylon.manifest.json and the typed client, then the production");
            println!("artifact: the server bundle, the client bundle, public/, and the manifest.");
            println!("`pylon start <dir>` runs the artifact without the source tree or");
            println!("node_modules. Settings: the `build` block of buildManifest in app.ts.");
            println!();
            println!("Options:");
            println!("  --out <dir>          Artifact directory (default: dist)");
            println!("  --compile            Also copy the pylon and bun binaries into <dir>/bin");
            println!("  --no-bundle          Write the manifest and client only (no artifact)");
            ...
            println!("  --no-static          Skip static route rendering");
