//! A zone shard: players with hit points, buffs, and cooldowns move between
//! zones, keeping all of it (see `Shard::transfer_out` and `transfer_in`).
//!
//! Build: `cargo build -p pylon-shard-guest --example zone --release
//! --target wasm32-unknown-unknown`. The runtime's WASM shard tests load the
//! result.
//!
