...
statecraft runs as a single binary behind a TLS-terminating reverse proxy.
...
STATECRAFT_MANIFEST=/etc/statecraft/statecraft.manifest.json

# Auth (MUST be set in non-dev)
STATECRAFT_ADMIN_TOKEN=<64+ random bytes, hex>
STATECRAFT_SESSION_DB=/var/lib/statecraft/sessions.db

# Client-facing
STATECRAFT_CORS_ORIGIN=https://your-app.com
STATECRAFT_CSRF_ORIGINS=https://your-app.com
...
```sh
STATECRAFT_JOBS_DB=/var/lib/statecraft/jobs.db  # durable job queue
STATECRAFT_OAUTH_GOOGLE_CLIENT_ID=...         # enable Google OAuth
STATECRAFT_OAUTH_GITHUB_CLIENT_ID=...         # enable GitHub OAuth
STATECRAFT_EMAIL_HTTP_URL=https://api.resend.com/emails  # or SMTP via transport config
```

Security hard requirements that fail to start:
- `STATECRAFT_CORS_ORIGIN=*` in non-dev mode is refused
- `STATECRAFT_DEV_MODE=true` with `STATECRAFT_ADMIN_TOKEN` unset is refused
...
```ini
# /etc/systemd/system/statecraft.service
[Unit]
Description=statecraft
After=network-online.target

[Service]
EnvironmentFile=/etc/statecraft/env
ExecStart=/usr/local/bin/statecraft serve
Restart=on-failure
RestartSec=5s
User=statecraft
Group=statecraft
...
```sh
systemctl enable --now statecraft
```

Backups: cron `statecraft backup /var/backups/statecraft/$(date +%F)` nightly.
Test restore quarterly per the test at `crates/runtime/tests/backup_restore.rs`.

## Shape 2: AWS ECS + Aurora

`deploy/terraform/modules/statecraft/` provisions:
...
Compiled with `--features postgres-live` and `STATECRAFT_DB_BACKEND=postgres`
+ `STATECRAFT_DB_URL=postgres://...`.
...
Starts on port 4321 with `STATECRAFT_DEV_MODE=true` defaults. Studio at
