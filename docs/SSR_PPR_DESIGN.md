...
Status: proposal · 2026-06-28 · Owner: TBD

## 1. Problem

Pylon's anonymous output cache (#277) is all-or-nothing. A render is cacheable
only if `computeCacheVerdict` passes every condition, including `!authTouched`,
...
The dominant real-world pattern is a public marketing page with a "Sign in / Dashboard"
nav in the layout. On `notbehind.com`, `app/layout.tsx:23` reads
...
2. `wantsStream = computeWantsStream(!!Loading, mod)`, derived statically from module exports
...
(`frontend.rs:698-704`), so query strings bypass the cache and authed requests never
...
**no Bun round-trip**. This cookie gate is what PPR must change; see §6.
...
PPR is buildable on react-dom without adopting full RSC/Flight; Pylon keeps
single-pass SSR and hydrated islands. Verify the API names and semantics against
the pinned React before implementation. The lock currently resolves react-dom
19.2.7; see O1.
...
- `resume(element, postponed, opts)` from `react-dom/server` (Web-streams; the
  ...
  subtrees.** It does not literally "render only the holes." Because React skips
  completed shell subtrees, dynamic boundaries dominate the per-request cost instead of the
  dynamic boundaries, not a full re-render.
- **Making a region a hole.** A per-request read (auth/cookies/…) must leave its
  ...
  stable mechanism does not depend on an unstable API: in prerender mode, the read
  returns a
  **never-resolving promise** (suspends), so the boundary never completes and
  ...
  if the pinned React exposes it. It is not a documented stable API, so the
...
1. Build the element tree as today, but with **prerender-mode proxies over every
   per-request input**, including inputs beyond auth. The current runtime threads
   `props.auth`
   ...
   leak. Proxy every per-request input, including `headers.host`, to prevent cache
   poisoning.
...
**Single-flight (open work).** The existing
...
re-render. See §3 for the exact execution model.
...
current buffered, streaming, and cache code paths without behavior changes.

**PPR shell verdict.** PPR keeps the other gates in
...
(under `PYLON_STRICT_FN_POLICIES`, serverData reads are auth-filtered and a shell
`serverData` read remains unshareable), and **no per-request read escaped
...
The cache stores the prelude. The hydration tail is generated per request.

### 5.1 The hydration tail is per-request
...
The tail is never cached. The cache holds the anonymous prelude; the tail is
...
- `frontend.rs`: needs a **separate PPR eligibility + read path** distinct from the
  static fast path. Today `cacheable_eligible` (`:698-704`) requires `GET` +
  ...
  *shell* to authed requests and resume the holes with their real auth. So add a PPR
  ...
  entry and calls the runner's resume entrypoint with
  ...
  `Cache-Control` for a PPR response is **`private, no-store`** because it is personalized.
  The caching happens inside Pylon, not at the CDN; see §8. Strip internal proof headers
  as today (`:1828`).
...
4. **Cookie/status from a hole is dropped** (the head commits with the prelude, the same
...
- For a coarse, binary dynamic region such as notbehind's signed-in state, use
  auth-bucketed variant caching. Render two anonymous
  variants keyed on session-cookie presence (via the new `props.session.exists`
  primitive that never reads or serializes identity; see Phase 0), cache both (Pylon ISR
  + a Cloudflare Cache Rule keyed on the cookie). That gets a true CDN `HIT`, no flash,
  no PPR machinery. It works only when the authed variant is user-agnostic (no
...
1. **Phase 0: auth-bucketed variant caching** (ships the notbehind win cheaply;
   ...
   cache = "auth-bucketed"`. This cannot be built on `props.auth`: any
   `auth.user_id` read flips `authTouched` and vetoes the cache (`ssr-runtime.ts:1799,
   1497`). It needs a new coarse primitive, such as `props.session.exists` (a boolean
...
4. **Phase 3: docs/skill.** The boundary contract, the PPR-vs-variant decision, and the
...
- **O3**: measure the crossover between resuming per request and rendering cheap shells
  live; PPR may not pay off for tiny pages.
- **O4**: verify that `serverData`/`use()` boundaries compose with auth holes without
  double-postpone.
