# Agent setup for `pylon`

This is a deepsec scanning workspace. Project `pylon` was just registered
(target: `..`). Setup is incomplete — `data/pylon/INFO.md`
...
2. **Fill in `data/pylon/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id pylon
pnpm deepsec process --project-id pylon
```
